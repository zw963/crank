...
module Foreman
  class Engine
    class CLI < Foreman::Engine
      ...
      private def pad_process_name(name)
        name.ljust(name_padding, " ")
      end

      private def map_colors
        colors = Hash(String, String).new("white")
        @names.values.each_with_index do |name, index|
          colors[name] = FOREMAN_COLORS[index % FOREMAN_COLORS.size]
        end
        colors["system"] = "bright_white"
        colors
      end

      private def proctitle(title)
        title
      end

      private def termtitle(title)
